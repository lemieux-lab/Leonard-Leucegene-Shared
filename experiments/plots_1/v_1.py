...
def hist_by_input_d(data, dataset, input_dims, by_input_dims_output_path):
    ...
    for in_d in tqdm(input_dims):
            ...
            descr_dict = {
                ...
                "CF-PCA": f"Clinical features (6) + PCA ({in_d})"
            }
...
def run(args):
    ...
    for dataset in datasets:
        ...
        for proj_type in proj_types:
            ...
            plt.plot(d["input_d"].astype(int),d["c_ind_tst"], label = proj_type)
        plt.hlines(xmin = 16, xmax = 18, y =data[data["proj_type"] == "LSC17"].c_ind_tst.mean(), color= 'k', linestyle = "--",label = "LSC17 mean")
        plt.scatter(np.ones(1000) * 17 , [data[data["proj_type"] == "LSC17"].c_ind_tst], color= 'k', s= 1,alpha = 0.2, label = "LSC17 distr")
        plt.ylim([0.5, 0.75])
        plt.legend()
        ...
        #plot_method_vs_method_by_input_pval(data, input_dims, ["PCA", "CF-PCA", "RSelect", "RPgauss"], by_methods_output_path)
        
        print("Plotting LSC17 vs method by input_d p_val difference")
        plot_LSC17_vs_method_by_input_pval(data, input_dims,  ["PCA", "CF-PCA", "RSelect", "RPgauss"], by_methods_output_path)